...
class Ensemble(SentimentModel):
    ...
    def __init__(self, cfg):
        ...
        """
        super(Ensemble, self).__init__(cfg)
        ...
        self.model1 = KoBERTClassifier(bertmodel).to(self.device)
        self.model2 = KoBERTClassifier(bertmodel, num_classes=44).to(self.device)
        if cfg['model_path'] is not None:
            self.model1.load_state_dict(torch.load(cfg['model_path'], map_location=self.device))
            
        if cfg['model2_path'] is not None:
            self.model2.load_state_dict(torch.load(cfg['model2_path'], map_location=self.device))
    ...
    def predict(self, x):
        ...
        with torch.no_grad():
          ...
          for token_ids, valid_length, segment_ids in dataloader:
              ...
              embedding1, _ = self.model1(token_ids, valid_length, segment_ids)
              embedding2, _ = self.model2(token_ids, valid_length, segment_ids)
              
              ensemble_embedding = (embedding1 + embedding2)/2
              
              out = self.model1.classifier(ensemble_embedding)
